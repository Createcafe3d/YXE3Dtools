...
def load_library(name):
    ...
    if os.environ.get('PEACHY_API_DLL_PATH'):
        ...
        try:
            dist = pkg_resources.get_distribution('YXE3DtoolsAPI')
            if python_64:
                dll_path = os.path.join(dist, 'YXE3D', 'dependancies', dependency_platform, 'amd64')
            else:
                dll_path = os.path.join(dist,'YXE3D', 'dependancies', dependency_platform, 'x86')
            logging.info("Loading usb dll via package resources")
        except Exception:
            current_path = os.path.dirname(__file__)
            if python_64:
                dll_path = os.path.join(current_path, '..', 'YXE3D', 'dependancies', dependency_platform, 'amd64')
            else:
                dll_path = os.path.join(current_path, '..', 'YXE3D', 'dependancies', dependency_platform, 'x86')
